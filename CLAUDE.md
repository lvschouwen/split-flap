...
- **System tab (#245):** netTask samples S3 vitals every 5 s into a mutex-guarded ring (`SystemStatsPolicy.h` pure + `SystemStats.cpp` glue); `GET /system/stats` = current + ~10 min history; I2C tx/err counters in `UnitBus.cpp` (sketch-protocol traffic only — idle polls and twiboot excluded), MQTT drops, SNTP sync age (`ClockService.cpp`).
