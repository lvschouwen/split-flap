...
- **Boot-rescue re-push (#343):** a member reply carrying `rescue:1` makes it a follower-image candidate even at the stored rev (`ClusterMemberRuntime.rescue`); a rejoin still beaconing burns the attempt (`RescueLooping` — the cap stops a poisoned store image); `rescue:true` in status members[] + a red member pill.
