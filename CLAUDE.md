...
- **MQTT + Home Assistant (#224):** v1's wire contract unchanged (22 discovery entities, LWT, five command topics, same device id) on espMqttClient, owned entirely by mqttTask — threading contract and per-mechanism detail in `MqttService.h`/`.cpp` headers, pure logic in `MqttHelpers.h` + `MqttLifecyclePolicy.h` (natively tested). `text/set` is a show-then-revert overlay gating clockTask; web transient text is still #219. Deviations from v1 in the spec (`2026-07-12-v2-mqtt-ha-slice.md`): no bus polls from the MQTT loop, NVS boot counter, `MQTT_DEVICE_MODEL` build flag.
