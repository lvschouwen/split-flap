...
An Arduino-based split-flap display. Firmware built with **PlatformIO** — each of the three sketches has its own `platformio.ini` and builds from the CLI. Host-side unit tests for pure-logic helpers live under `ESPMaster/test/`. CI in `.github/workflows/build.yml` builds all envs and runs the native tests.
