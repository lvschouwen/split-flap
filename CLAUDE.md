...
**Firmware status (2026-04-22):** `firmware/ESPMaster/` (ESP8266) is in **maintenance mode** — security and bug fixes only, no new features. Known-good build tagged `v-esp8266-final`. Active feature development is on the ESP32-S3 + ESP32-H2 stack on the Master v2 Rev B PCB — `firmware/MasterS3/` and `firmware/MasterH2/` (both in progress). See meta issue #58 for the migration plan and `PCB/MASTER_V2/` for the hardware spec.
