...
- **`firmware/MasterS3/`** — *In progress (#58).* ESP32-S3-WROOM-1-N16R8 firmware on the Master v2 Rev B PCB. Standard PlatformIO layout (`src/main.cpp`, no Arduino preprocessor magic). Uses pioarduino fork of `platform-espressif32` for arduino-esp32 v3.x. USB-CDC on boot. Wokwi config + custom TCA9548A chip at `firmware/MasterS3/wokwi.toml`, `firmware/MasterS3/diagram.json`, `firmware/MasterS3/chips/tca9548a/`. **S3↔H2 UART link** (#67) lives inline in `main.cpp`: Serial1 at 921600 8N1 on rx=IO18/tx=IO17; sends PING every 1 s and logs PONG RTT; link up/down detection within 3 s. H2 control pins IO11 (RESET), IO12 (IRQ), IO13 (BOOT_SEL) initialised to their idle-safe state — real use is Phase 4 OTA.
