...
- **`UnitBus.cpp` is the ONLY Wire toucher on v2, and displayTask its only caller** (SDA 8 / SCL 9, 100 kHz — #383 reverted #375's 400 kHz; Nano TWI slaves follow the master clock, no unit reflash).
...
`Unit.ino` (config, globals, setup/loop) + siblings `UnitI2CProtocol.ino` (TWI ISRs + addressing) and `UnitMotion.ino` (stepper + hall calibration) — a one-TU concat model, main sketch placed first. I2C address: four DIP pins + `I2C_ADDRESS_BASE` (DIP 0000 → 0x01; 0x00 reserved for general call), or EEPROM-provisioned address with DIP fallback — twiboot still listens on the DIP-derived address, so over-I2C reflash requires EEPROM == DIP. 28BYJ-48 via ULN2003, KY-003 hall homing, per-unit EEPROM step offset clamped to ±`SFP_OFFSET_LIMIT_STEPS` on both protocol sides. Revolution odometer (#231): every drum move funnels through `stepCounted()`; ring policy pure in `UnitOdometer.h` (`pio test -e native` runs here too). EEPROM layout + block integrity pure in `UnitEeprom.h` (#406, day-0, one version byte — every block checksummed and every failure degrades to a safe default; the ring sits last so new scalars never move it). Lifetime health (what the unit remembers about its own decline across power cycles) reads back via `GET_LIFETIME` → `UnitLifetime.h`. **Feature gates (EEPROM byte 11, `UNIT_GATE_*`):** the two motion changes ship OFF so one fleet reflash can be proven incrementally; `SET_GATES` (0x99) / `POST /unit/gates` flips them over the wire on both row masters, verified by a `GET_LIFETIME` read-back, and the unit refuses bits it has no code for. Idle hall consistency check (#268, behind `UNIT_GATE_IDLE_HALL_CHECK`): pure `UnitIdleHall.h` predicts the hall reading from the parked position and arms the existing #263 re-home on a debounced contradiction — coverage is partial by physics (see the header). Calibration/provisioning was driven from the master's web UI, which no longer exists — the DisplayCommand ops behind it are intact and currently unreachable. Probe quirk: twiboot pinned alive by `isUnitInBootloader()` probe — the 1500 ms pre-probe delay is load-bearing.

## v2 — Master (ESP32-S3)

One line per mechanism: what it is + issue # + the file(s) whose headers carry the detail. Specs in `docs/superpowers/specs/`.

> **⚠️ THE MASTER HAS NO NETWORK SURFACE AT ALL. Do not flash it to a wall board.**
>
> The web UI and the entire HTTP layer were deleted so the API and the UI can be rewritten from the ground up rather than carried. Gone: every page and asset, and every endpoint — settings, units, system, firmware/OTA, maintenance, log, wifi, and the whole cluster wire (leader, follower, discovery, rollout, follower-image relay, HMAC). `AsyncWebServer` is no longer instantiated.
>
> **What this means operationally: there is no OTA. A board flashed with this build can only be recovered over USB.** The bench boards are deliberately left on their last good image. **Nothing gets flashed until the new API ships with OTA in it** — that is the minimum bar for the first deploy, not a later milestone.
>
> Still present, because they are the product rather than a protocol: the display domain, units/I2C, settings/NVS, clock/NTP, WiFi join, status LED, flash log, OTA *service* (`OtaService`, `FactorySlot` — the machinery, with no endpoint in front of it), MQTT/Home Assistant, and the bundled unit image. `ContentState.cpp` now owns the runtime display content the web layer used to own.
>
> `firmware/v2/FollowerEsp01` still builds but is **orphaned** — it is a cluster follower with no leader left to follow. Decide its fate as part of the new design.

- **Tasks (#187):** dual-core FreeRTOS — display domain core 1, network core 0; command queue in, mutex-copied snapshots out. Contract in `Tasks.h`; TU family (#352): `Tasks.cpp` (IPC + lifecycle + core-0 wrappers), `DisplayTask.cpp` (core-1 display domain, per-opcode exec* helpers), `ClockTask.cpp` (1 Hz ticker), seams in `TasksInternal.h` (family-only include). PSRAM-preferred buffers via `LargeAlloc.h`. **Display source (#403):** every `makeShowTextCommand` states its producer (`DisplaySource` — web/mqtt/clock/leader, no default, required arg); the snapshot keeps the CURRENT text's source + a millis stamp, surfaced as `displaySource`/`displaySourceAge` in `/settings`. The re-show opcodes (ResetUnits/ReflashUnits) leave it alone on purpose — restoring text is not authoring it. The producer rides `clusterLeaderSubmitText()` so a leader's own row credits the caller, not itself.
...
- **Content state (replaces the web layer's ownership):** `ContentState.h`/`.cpp` owns the runtime display content — device mode, the retained message and its `DisplaySource` (#403), alignment, flap speed — plus the staged graceful reboot and the timezone/reset-reason reads. This used to live inside `WebEndpoints.cpp`, which meant the HTTP surface owned display state and clockTask/mqttTask reached into it; that coupling is why replacing the API meant touching the display domain. **A new API sits ON this module — it must not own state again.** Mutex-guarded, snapshot-copy discipline identical to `DisplaySnapshot`; drained by `contentStateTick()` on netTask.
...
- **OTA (#190/#305) — SERVICE ONLY, no endpoint in front of it:** the A/B install machinery is intact and unused. It took a stream onto the inactive slot with a mandatory MD5; images boot `PENDING_VERIFY`, confirmed **pre-inrush at the end of `setup()`** (netif-up is a fallback bar only — a later confirm loses to the boot-inrush brownout). Detail in `OtaService.h`/`.cpp` + `OtaStatus.h`.
...
- **Flash log (#206):** LittleFS tee of the serial stream on the `storage` partition — `FlashLog.h` + `FlashLogPolicy.h`; writer discipline in Hard rules. Nothing reads it back on-device now that the HTTP layer is gone; the new API should expose it, since a previous boot's log is how a crash gets diagnosed.
...
### Cluster (epic #270) — DELETED, to be redesigned

The N-row wall (leader/follower over LAN HTTP+JSON, HMAC-signed wire, sequential fan-out, fleet rollout, follower-image relay, discovery, single-pane digest, promote/demote) was removed with the rest of the HTTP layer. Nothing of it remains on the Master.

The specs stay on disk as prior art, not as a description of the code: `2026-07-13-multi-display-cluster-design.md`, `2026-07-14-cluster-single-pane-design.md`, `2026-07-14-v2-esp01-follower-firmware-relay.md`. `ClusterHmac.h` survives in `shared/` because FollowerEsp01 still compiles against it.

Worth carrying into whatever replaces it, because these were the expensive lessons rather than the code: one flap size for the whole wall so a narrow row reads as narrow; a shared commit clock so rows flip together; strict sequencing for fleet updates with a health gate and an attempt cap; and a monotonic replay mark alongside the timestamp window, because an NTP window alone is not a replay defence.
