...
Four active firmware targets: **ESPMaster** (ESP8266 ESP-01, current production master, last released tag `v-esp8266-final`), **Unit** (Arduino Nano per-flap), **MasterS3** + **MasterH2** (ESP32-S3 + ESP32-H2 port on the Master v2 Rev B PCB, in progress; see meta issue #58 and `PCB/MASTER_V2/`).
