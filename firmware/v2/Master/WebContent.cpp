...
void webContentRegister(AsyncWebServer& server) {
  ...
  // The Wall Console (#399) is the whole UI: the page is the wall, and the
  // wall is the navigation. The pre-#396 five-tab panel was deleted rather
  // than parked on a second route — two front doors would have been the
  // hybrid the redesign set out to end.
  server.on("/", HTTP_GET, [](AsyncWebServerRequest* request) {
    serveGzipAsset(request, "text/html", CONSOLE_HTML_GZ, CONSOLE_HTML_GZ_LEN);
  });
  // Firmware constants the console needs but cannot #include — the unit
  // alphabet, drum geometry, status-flag bits — generated from the headers
  // that own them at bake time (#399). The console keeps no copy of any of
  // it, so there is nothing that can drift from the hardware.
  server.on("/constants.js", HTTP_GET, [](AsyncWebServerRequest* request) {
    serveGzipAsset(request, "application/javascript", CONSTANTS_JS_GZ,
                   CONSTANTS_JS_GZ_LEN);
  });
  server.on("/console.css", HTTP_GET, [](AsyncWebServerRequest* request) {
    serveGzipAsset(request, "text/css", CONSOLE_CSS_GZ, CONSOLE_CSS_GZ_LEN);
  });
  server.on("/console.js", HTTP_GET, [](AsyncWebServerRequest* request) {
    serveGzipAsset(request, "application/javascript", CONSOLE_JS_GZ,
                   CONSOLE_JS_GZ_LEN);
  });
  server.on("/console-detail.js", HTTP_GET, [](AsyncWebServerRequest* request) {
    serveGzipAsset(request, "application/javascript", CONSOLE_DETAIL_JS_GZ,
                   CONSOLE_DETAIL_JS_GZ_LEN);
  });
