...
// Pure per-field validators for POST / (#153), natively tested in
// test/test_settings_validation.
#include "SettingsValidation.h"

//Shared 502 response for one-shot unit commands whose Wire write failed at
//the bus level (#159).
static void sendWireError(AsyncWebServerRequest* request, int status) {
...
void registerWebEndpoints() {
    ...
    //(generated by build_assets.py into WebAssets.h). Gzipped entries go
    //through serveGzipAsset() (ServiceBootModes.ino), which also carries
    //the shared no-cache revalidation policy.
    ...
    webServer.on("/script.js", HTTP_GET, [](AsyncWebServerRequest * request) {
      serveGzipAsset(request, "application/javascript", SCRIPT_JS_GZ, SCRIPT_JS_GZ_LEN);
    });
      ...
      for (int paramIndex = 0; paramIndex < params; paramIndex++) {
        ...
        if (p->isPost()) {
          ...
          if (p->name() == PARAM_ALIGNMENT) {
            ...
            if (isValidAlignmentValue(receivedValue)) {
          ...
          if (p->name() == PARAM_DEVICEMODE) {
            String receivedValue = p->value();
            if (isValidDeviceModeValue(receivedValue)) {
          ...
          if (p->name() == PARAM_FLAP_SPEED) {
            String receivedValue = p->value();
            if (isValidFlapSpeedValue(receivedValue)) {
          ...
          if (p->name() == PARAM_TIMEZONE) {
            String receivedValue = p->value();
            if (isValidTimezoneValue(receivedValue, LEN_TIMEZONE)) {
          ...
          if (p->name() == PARAM_MQTT_HOST || p->name() == PARAM_MQTT_USER) {
            ...
            String receivedValue = p->value();
            receivedValue.trim();
            bool valid = isValidMqttHostOrUserValue(receivedValue, isHost ? LEN_MQTT_HOST : LEN_MQTT_USER);
          ...
          if (p->name() == PARAM_MQTT_PORT) {
            String receivedValue = p->value();
            receivedValue.trim();
            if (isValidMqttPortValue(receivedValue, LEN_MQTT_PORT)) {
          ...
          if (p->name() == PARAM_MQTT_PASSWORD && p->value().length() > 0) {
            String receivedValue = p->value();
            if (isValidMqttPasswordValue(receivedValue, LEN_MQTT_PASSWORD)) {
